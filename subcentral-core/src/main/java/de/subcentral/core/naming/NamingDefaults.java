...
public class NamingDefaults {
	...
	private static final DelegatingNamingService NORMALIZING_NAMING_SERVICE = createNormalizingNamingService(
			NAMING_SERVICE);
	...
	static {
		...
		// PropToStringService
		PROP_TO_STRING_SERVICE.getTypeToStringFns().put(Year.class,
				(Year y) -> DateTimeFormatter.ofPattern("uuuu", Locale.US).format(y));
		...
		PROP_TO_STRING_SERVICE.getPropToStringFns().put(Episode.PROP_NUMBER_IN_SERIES,
				(Integer n) -> String.format("E%02d", n));
		PROP_TO_STRING_SERVICE.getPropToStringFns().put(Episode.PROP_NUMBER_IN_SEASON,
				(Integer n) -> String.format("E%02d", n));
		// Release
		...
		Config configWithRlsNameFormatter = new Config(PROP_TO_STRING_SERVICE, separations,
				Separation.DEFAULT_SEPARATOR, RELEASE_NAME_FORMATTER);
		Config configWithSubAdjNameFormatter = new Config(PROP_TO_STRING_SERVICE, separations,
				Separation.DEFAULT_SEPARATOR, SUBTITLE_ADJUSTMENT_NAME_FORMATTER);
		...
		SUBTITLE_ADJUSTMENT_NAMER = new SubtitleAdjustmentNamer(configWithSubAdjNameFormatter, RELEASE_NAMER);
	...
	private static Function<String, String> initSubtitleAdjustmentNameFormatter() {
		return StandardizingDefaults.ACCENT_REPLACER.andThen(StandardizingDefaults.ALNUM_DOT_HYPEN_UNDERSCORE_REPLACER)
				.andThen(StandardizingDefaults.DOT_HYPHEN_DOT_REPLACER);
	}
	...
	public static DelegatingNamingService createNormalizingNamingService(NamingService namingService) {
		return new DelegatingNamingService(namingService.getDomain() + ".normalizing", namingService,
