...
public class OrlyDb
{
	...
	private static final NamingServiceImpl	ORLYDB_QUERY_ENTITY_NAMING_SERVICE	= new NamingServiceImpl();
	static
	{
		// initialize host url
		try
		...
		epiNamer.setWholeNameReplacer(ORLYDB_QUERY_ENTITY_REPLACER);

		MovieNamer movieNamer = new MovieNamer();
		movieNamer.setWholeNameReplacer(ORLYDB_QUERY_ENTITY_REPLACER);

		MediaReleaseNamer mediaRlsNamer = new MediaReleaseNamer();
		mediaRlsNamer.setWholeNameReplacer(ORLYDB_QUERY_ENTITY_REPLACER);
