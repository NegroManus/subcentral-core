...
public class XRel
{
	...
	private static final NamingServiceImpl	XREL_QUERY_ENTITY_NAMING_SERVICE	= new NamingServiceImpl();
	static
	{
		// initialize host url
		try
		...
		epiNamer.setWholeNameReplacer(XREL_QUERY_ENTITY_REPLACER);

		MovieNamer movieNamer = new MovieNamer();
		movieNamer.setWholeNameReplacer(XREL_QUERY_ENTITY_REPLACER);

		MediaReleaseNamer mediaRlsNamer = new MediaReleaseNamer();
		mediaRlsNamer.setWholeNameReplacer(XREL_QUERY_ENTITY_REPLACER);
